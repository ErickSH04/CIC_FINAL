...
public class ventanaCitasCompletadasPac extends javax.swing.JFrame {

    private static String usuarioId;
    private static Statement stmt;
    public static Connection con;

    public ventanaCitasCompletadasPac(String usuarioId) {//String usuarioId
    ...
    private void initComponents() {
        ...
        tblCita.setModel(new javax.swing.table.DefaultTableModel(
            ...
            new String [] {
                "Fecha", "Hora", "Nombre", "Apellido1", "ID de Medico"
            }
        ) {
        ...
        jLabel3.setIcon(new javax.swing.ImageIcon(getClass().getResource("/imagenes/citaCompletada.png"))); // NOI18N
        ...
        lblBuscar.setText("Buscar medico");
   ...
   public static String obtenerIdPac(String correo){
       con = ConexionSQL.ConexionSQLServer();
        Statement stmt2;
        ...
        try {
            ...
            String idPac = "select pac.numeroSeguro \n" +
                            "from paciente pac\n" +
                            "inner join usuario u on (pac.Correo = u.correo)\n" +
...
private void buscarPorFecha() {
    ...
    // ID del paciente
    String idPaciente = obtenerIdPac(this.usuarioId);

    // ID del médico desde el campo de texto
    String idMedico = txtBuscar.getText().trim();

    int nDia = cmbDia.getSelectedIndex();
    int nMes = cmbMes.getSelectedIndex();

    // ----------------------------------------
    // SI NO HAY MÉDICO → traer TODAS las citas del paciente
    // ----------------------------------------
    String query;

    if (idMedico.isEmpty()) {
        query =
            "SELECT cit.fecha, cit.hora, med.nombreMed, med.apellido1, med.idMedico " +
            "FROM citasCompletadas cit " +
            "INNER JOIN medico med ON med.idMedico = cit.idMedico " +
            "WHERE cit.numeroSeguro = '" + idPaciente + "' ";
    } else {
        // Query cuando sí se especifica médico
        query =
            "SELECT cit.fecha, cit.hora, med.nombreMed, med.apellido1, med.idMedico " +
            "FROM citasCompletadas cit " +
            "INNER JOIN medico med ON med.idMedico = cit.idMedico " +
            "WHERE cit.numeroSeguro = '" + idPaciente + "' " +
            "AND cit.nombreMed = '" + idMedico + "' ";
    }

    // ---------------------------------------------
    // FILTROS OPCIONALES POR MES Y DÍA
    // ---------------------------------------------
    if (nMes > 0) {
    ...
    // ---------------------------------------------
    // SI NO ELIGIÓ NI MES NI DÍA → mostrar todas
    // ---------------------------------------------
    if (nDia == 0 && nMes == 0) {
        llenarCitas(); 
        return;
    ...
    private void buscarMedico(String nombre){
            ...
            query = "select cit.fecha, cit.hora, med.nombreMed, med.apellido1, med.apellido2, med.idMedico\n" +
                "from citasCompletadas cit\n" +
                "inner join medico med on (med.idMedico = cit.idMedico)\n" +
                "where med.nombreMed ="+ "'"+busqueda+"';";
            try {
                stmt = con.createStatement();
    ...
    public void llenarCitas() {
        ...
        m.setRowCount(0);
        Statement stmt2;
        ResultSet resultadoPac;
        String identificadorPac="";
        
        try {
            
            stmt2 = con.createStatement();
            String idMed = "select pac.numeroSeguro \n" +
                            "from paciente pac\n" +
                            "inner join usuario u on (pac.Correo = u.correo)\n" +
                            "where pac.Correo = "+"'"+this.usuarioId+"'";
            
            
            resultadoPac = stmt2.executeQuery(idMed);
            
            while(resultadoPac.next()){
                identificadorPac = resultadoPac.getString("numeroSeguro");
            }
       
            stmt = con.createStatement();
            String query = "select cit.fecha, cit.hora, med.nombreMed, "+
                            "med.apellido1, med.NumCedula\n" +
                            "from citasCompletadas cit\n" +
                            "inner join medico med on (med.idMedico = cit.idMedico)\n" +
                            "where cit.numeroSeguro="+"'"+identificadorPac+"'"+";";
            ...
            while (result.next()) {
                ...
                R[4] = result.getObject("NumCedula");
                m.addRow(R);
            }
        } catch (SQLException ex) {
            java.util.logging.Logger.getLogger(ventanaCitasMed.class.getName()).log(java.util.logging.Level.SEVERE, null, ex);
